...
import type { PaymentMethod, PriceTier, ProductJSON } from "@/lib/types";
...
function priceForTier(product: ProductJSON, tier: PriceTier): number {
...
export default function PosPage() {
  ...
  function addToCart(product: ProductJSON) {
    setCart((prev) => {
      ...
      return [...prev, { product, quantity: 1, priceTier: "retail" }];
    });
  }
  ...
  const total = useMemo(
    () => cart.reduce((sum, line) => sum + priceForTier(line.product, line.priceTier) * line.quantity, 0),
    [cart]
  );
  ...
  async function completeSale() {
    ...
    const params = search ? `?search=${encodeURIComponent(search)}` : "";
    const refreshed = await fetch(`/api/products${params}`);
          ...
          {products.map((product) => (
            ...
            >
              ...
              <span className="text-xs text-zinc-500">
                Stock: {product.quantityInStock} · Retail ${product.retailPrice.toFixed(2)}
              </span>
            </button>
          ))}
          {products.length === 0 && (
              ...
              <div key={line.product._id} className="border-b border-zinc-100 pb-3 last:border-0">
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateLine(line.product._id, { priceTier: e.target.value as PriceTier })
                    }
                    className="flex-1 rounded border border-zinc-300 px-2 py-1 text-sm"
                  >
                    {(Object.keys(TIER_LABEL) as PriceTier[]).map((tier) => (
                      <option key={tier} value={tier}>
                        {TIER_LABEL[tier]} (${priceForTier(line.product, tier).toFixed(2)})
                      </option>
                    ))}
                  </select>
                </div>
                <div className="mt-1 text-right text-sm text-zinc-600">
                  ${(priceForTier(line.product, line.priceTier) * line.quantity).toFixed(2)}
                </div>
              </div>
            ))}
          </div>
